...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("does not pass the selected agent request into the provider turn", () =>
    Effect.gen(function* () {
      yield* setup
      ...
      yield* agents.transform((editor) =>
        editor.update(AgentV2.ID.make("build"), (agent) => {
          agent.mode = "primary"
          agent.request.headers = { "x-agent": "agent" }
          agent.request.body = { temperature: 0, topP: 1, seed: 1234 }
        }),
      )
      ...
      expect(build.request).toEqual({
        ...
        body: { temperature: 0, topP: 1, seed: 1234 },
      })

      const session = yield* SessionV2.Service
      yield* session.prompt({ sessionID, prompt: Prompt.make({ text: "Use agent request" }), resume: false })
      requests.length = 0
      response = []
      yield* session.resume(sessionID)
      ...
      expect(requests[0]?.providerOptions).toEqual({ openai: { promptCacheKey: sessionID } })
      expect(requests[0]?.model.route.defaults.generation).toBeUndefined()
      expect(requests[0]?.model.route.defaults.http).toBeUndefined()
    }),
  )

  it.effect("keeps model route request defaults when agent request values conflict", () =>
    Effect.gen(function* () {
      yield* setup
      currentModel = Model.make({
        ...
        route: OpenAIChat.route.with({
          ...
          generation: { temperature: 0.7, topP: 0.9, seed: 42 },
      ...
      const session = yield* SessionV2.Service
      yield* session.prompt({ sessionID, prompt: Prompt.make({ text: "Use model request" }), resume: false })
      requests.length = 0
      response = []
      yield* session.resume(sessionID)
      ...
      expect(requests[0]?.http).toBeUndefined()
      expect(requests[0]?.providerOptions).toEqual({ openai: { promptCacheKey: sessionID } })
      expect(requests[0]?.model.route.defaults.generation).toMatchObject({
        temperature: 0.7,
        topP: 0.9,
        seed: 42,
      })
      expect(requests[0]?.model.route.defaults.providerOptions).toEqual({
        openai: { store: false, serviceTier: "priority" },
      })
      expect(requests[0]?.model.route.defaults.http?.body).toEqual({ custom_extension: { enabled: true } })
      expect(requests[0]?.model.route.defaults.http?.headers).toEqual({ "x-model": "model" })
    }),
  )
