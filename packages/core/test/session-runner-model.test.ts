...
describe("SessionRunnerModel", () => {
  ...
  it.effect("maps catalog request bodies into route HTTP defaults without lowering generation fields", () =>
    Effect.gen(function* () {
      const resolved = yield* SessionRunnerModel.fromCatalogModel(
        ModelV2.Info.make({
          ...model({ type: "aisdk", package: "@ai-sdk/openai", url: "https://openai.example/v1" }),
      ...
      expect(resolved.route.defaults.headers).toEqual({ "x-model": "model" })
      expect(resolved.route.defaults.generation).toBeUndefined()
      expect(resolved.route.defaults.providerOptions).toEqual({ openai: { store: false } })
      ...
      const failure = yield* LLMClient.prepare(LLM.request({ model: resolved, prompt: "Hello" })).pipe(Effect.flip)
      expect(failure.message).toContain("http.body cannot overlay protocol-owned field(s)")
      expect(failure.message).toContain("temperature")
