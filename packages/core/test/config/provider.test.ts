...
describe("ConfigProviderPlugin.Plugin", () => {
  ...
  it.effect("keeps generation-like request fields in provider and model bodies", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                providers: {
                  custom: {
                    api: { type: "aisdk", package: "@ai-sdk/openai", url: "https://example.test/v1" },
                    request: {
      ...
      const model = required(yield* catalog.model.get(providerID, modelID))
      expect(model.request.headers).toEqual({ "x-provider": "provider", "x-model": "model" })
